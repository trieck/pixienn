...
| `centernet-kitti` | KITTI object-detection split generated by `shell/kitti.sh` |

Smoke presets are useful for correctness and overfitting tests, but their metrics
are not publication benchmarks.

KITTI setup downloads the official left-camera images and training labels, then
converts the eight labeled object categories (excluding `DontCare`) to normalized
Darknet annotations under `resources/data/kitti`:

```bash
./shell/kitti.sh
./shell/check-training-data.sh resources/cfg/centernet-kitti-cfg.yml
```
