...
float BurnInLRPolicy::update(int batchNum)
{
    currentLR_ = initialLR_ * std::pow(static_cast<float>(batchNum) / batches_, power_);

    return currentLR_;
}

float BurnInLRPolicy::LR() const noexcept
{
    return currentLR_;
}

void BurnInLRPolicy::reset()
{
