...
public:
    ...
    void update(V& w, V& d, V& m, V& v, bool applyDecay);
private:
...
template<Device D>
void Adam<D>::update(V& w, V& d, V& m, V& v, bool applyDecay)
{
    auto invBatch = 1.0f / batch_;
    auto status = cublasSscal(ctxt_, d.size(), &invBatch, d.data(), 1);
    ...
    status = cublasSscal(ctxt_, m.size(), &beta1_, m.data(), 1);
    PX_CHECK_CUBLAS(status);
